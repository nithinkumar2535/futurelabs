...
document.addEventListener("DOMContentLoaded", function () {
  ...
  fetch(`${baseUrl}/api/v1/category/lessPrice/get`)
  ...
  if (activeTabName) {
    ...
    fetchTabContent(tabs[0].name);
      ...
      function fetchTabContent(tabName) {
        tabContent.innerHTML = `<div>Loading...</div>`; // Show loading indicator
        fetch(`${baseUrl}/api/v1/tests/get/lessPrice/${encodeURIComponent(tabName)}`)
          .then((response) => response.json())
          .then((Data) => {
