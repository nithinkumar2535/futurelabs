...
const addCategory = asyncHandler (async (req, res) => {
    ...
    const existingName = await WomenAge.findOne({name})
    ...
    const Category = await WomenAge.findById(category._id)
...
const updateCategory = asyncHandler( async (req, res) => {
    ...
    const updatedCategory = await WomenAge.findByIdAndUpdate(
        id,
        {selected}, 
        {new: true}
    )
...
const getAllCategory = asyncHandler( async (req, res) => {
    const category = await WomenAge.find()
...
const deleteCategory = asyncHandler(async (req, res) => {
    ...
    const category = await WomenAge.findById(id)
    ...
    await WomenAge.findByIdAndDelete(id)
...
const getSelectedCategories = asyncHandler(async (req, res) => {
    const selectedCategories = await WomenAge.find({ selected: true });
