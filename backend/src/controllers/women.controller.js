...
import { WomenPackages } from '../models/women.model.js'
import fs from 'fs'
...
const addCategory = asyncHandler (async (req, res) => {
    ...
    const existingName = await WomenPackages.findOne({name})
    
        if (existingName) {
            throw new ApiError(409, "Name alraedy exist")
        }

    const category = new WomenPackages({
        name,
    ...
    const Category = await WomenPackages.findById(category._id)
...
const updateCategory = asyncHandler( async (req, res) => {
    ...
    const updatedCategory = await WomenPackages.findByIdAndUpdate(
        id,
        {selected}, 
        {new: true}
    )
...
const getAllCategory = asyncHandler( async (req, res) => {
    const category = await WomenPackages.find()
...
const deleteCategory = asyncHandler(async (req, res) => {
    ...
    const category = await WomenPackages.findById(id)
    ...
    await WomenPackages.findByIdAndDelete(id)
...
const getSelectedCategories = asyncHandler(async (req, res) => {
    const selectedCategories = await WomenPackages.find({ selected: true });
